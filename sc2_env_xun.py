...
class SC2Env_xun(environment.Base):
  """A Starcraft II environment.
...
#        pdb.set_trace()                  
  ...
  def _observe(self, target_game_loop):
    ...
    if self._score_index >= 0:  # Game score, not win/loss reward.
      ...
      if _episode_steps == 0:  # First reward is always 0.
    ...
    if self._episode_steps >= self._episode_length-8:   #modified by xun
...
#                   [o["score_cumulative"][0] for o in self._agent_obs])
      #pdb.set_trace()
      # Modified by xun, only works for the drone. 2021
      logging.info(("Episode %s finished after %s game steps. "
                    "Outcome: %s, reward: %s, kill_number_units: %s"),
                   self._episode_count, self._episode_steps, outcome, reward,
                   [o["score_cumulative"][5]/50 for o in self._agent_obs])
